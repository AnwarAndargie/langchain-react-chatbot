...
def search_tavily(
    ...
    *,
    ...
    http_proxy = (settings.tavily_http_proxy or "").strip()
    https_proxy = (settings.tavily_https_proxy or "").strip()
    ...
    try:
        ...
        if exc_name == "ForbiddenError" or "403" in msg or "forbidden" in msg.lower() or "api_key" in msg.lower() or "auth" in msg.lower() or "401" in msg:
            logger.warning("Tavily API 403/Forbidden or auth failed: %s", exc_name)
            user_message = "Web search access denied (403). Check TAVILY_API_KEY and Tavily account access."
